  ...
  return (
    <Card className="card-glow bg-bg-card border border-border-subtle">
      ...
      <CardContent className="px-3 pb-3 text-xs text-text-muted">
        ...
        <Button size="sm" variant="primary" onPress={load} isDisabled={loading} className="mb-2">
          {loading ? "加载中…" : "加载多周期表"}
        </Button>
            ...
            {intervals.map((iv) => {
              ...
              return (
                <div key={iv} className="flex items-center gap-2 p-1.5 rounded bg-surface-hover">
                  ...
                  <Chip size="sm" variant="soft" color={v.color}>{v.text}</Chip>
                  <span className="text-[10px]">笔{r.bis?.length || 0} 段{r.segments?.length || 0}</span>
                  <Button size="sm" variant="ghost" className="ml-auto text-[10px]" onPress={() => openInChart(iv)}>
                    主图打开
                  </Button>
                </div>
              );
